---
name: blkout-community-media
description: Produce community podcasts, video content, and audio for BLKOUT UK. Covers participant briefing, consent frameworks, transcription, editing, storyboarding, video generation, and editorial stewardship. Formats include Hard Pressed (panel), Relay/Baton (dilemma chain), Theme + Discussion, and Decolonised Discs. Uses X-Men 97 illustration style for AI-generated visuals. Triggers on podcast production, recording, transcription, video storyboard, participant briefing, or media consent questions.
---
...
See `templates/email-templates.md` for ready-to-use emails.
...
```bash
python scripts/package_export.py PROJECT_DIR --output EXPORT_DIR --format remotion
```
...
**Audio files:** Google Drive (linked from Track Number column in sheet)
...
See `references/uk-copyright-guide.md` for full legal guidance.
...
| `package_export.py` | Assembly prep | project dir | export/ folder |
